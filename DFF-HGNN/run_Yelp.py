...
from Yelp.yelp_dataloader import load_Yelp_data
import numpy as np
import random
...
def main(args):
    NS, features, labels, num_classes, train_idx, val_idx, test_idx, type_mask, Relation_features= load_Yelp_data()
    ...
    early_stopping = EarlyStopping(patience=args['patience'], verbose=True, save_path='checkpoint/checkpointTest_{}.pt'.format('Yelp'))
    ...
    for epoch in range(args['num_epochs']):
        ...
        if early_stopping.early_stop:
            ...
            break
    print('\ntesting...')
    model.load_state_dict(torch.load('checkpoint/checkpointTest_{}.pt'.format('Yelp')))
    ...
    plt.legend()
    plt.savefig("Yelp_" + "分类图.png", dpi=1000,
                bbox_inches='tight')
    plt.show()
