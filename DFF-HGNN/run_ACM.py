...
from acm.acm_dataloader import  load_ACM_data
import numpy as np
import random
...
def main(args):
    NS, features, labels, num_classes, train_idx, val_idx, test_idx, type_mask, Relation_features= load_ACM_data()
    ...
    early_stopping = EarlyStopping(patience=args['patience'], verbose=True, save_path='checkpoint/checkpointTest_{}.pt'.format('ACM'))
    ...
    print('\ntesting...')
    model.load_state_dict(torch.load('checkpoint/checkpointTest_{}.pt'.format('ACM')))

    model.eval()
    logits, h = model(features, NS, Relation_features)
    evaluate_results_nc(h[test_idx].detach().cpu().numpy(), labels[test_idx].cpu().numpy(), int(labels.max()) + 1)   # 使用SVM评估节点
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_decay', default=0.00005, help='权重衰减')
