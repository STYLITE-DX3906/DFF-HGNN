...
from freebase.freebase_dataloader import  load_freebase
import numpy as np
import random

# from RAS_HGN import RAS_HGN
from MLP_Mix import MLP
...
def main(args):
    label, num_classes, train, val, test, feature, feature_dim, Relation_feature, Relation_feature_dim = load_freebase()
    ...
    for cur_repeat in range(args['repeat']):
        ...
        model = MLP(args["hidden_units"], num_classes, args['feat_drop'], feature_dim, Relation_feature_dim)
        ...
        for epoch in range(args['num_epochs']):
            ...
            model.train()
            logits, h = model(feature, Relation_feature)
        ...
        model.eval()
        logits, h = model(feature, Relation_feature)
        svm_macro, svm_micro, nmi, ari = evaluate_results_nc(h[test].detach().cpu().numpy(), label[test].cpu().numpy(),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Freebase')
    parser.add_argument('--dataset', default='Freebase_R拼A', help='数据集')
